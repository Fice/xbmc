...
EVENT_RESULT CGUIBaseContainer::OnMouseEvent(const CPoint &point, const CMouseEvent &event)
{
  ...
  { // release exclusive access
    ...
    if (event.m_state == 1  && CanDrag() && HitTest(point))
    {
      ...
      bool dropable = IsReorderable(); //TODO:
...
void CGUIBaseContainer::MoveItemInternally(int pos, int newPos)
{
  m_items.Move(pos, newPos-pos);
    //TODO: use m_items.Move(); and remove this function!
  /*
  ...
  int direction = (pos>newPos) ? -1 : 1;
  for(int i=pos; i!=newPos; i+=direction)
  {
    CGUIListItemPtr copy = m_items[i];
    m_items[i] = m_items[i+direction];
    m_items[i+direction] = copy;
  }*/
}
...
bool CGUIBaseContainer::OnClick(int actionID)
{
  ...
  if (actionID == ACTION_SELECT_ITEM || actionID == ACTION_MOUSE_LEFT_CLICK)
  {
    ...
    { // "select" action
      ...
      {
        CGUIStaticItemPtr item = boost::shared_polymorphic_downcast<CGUIStaticItem>(m_items[selected]);
